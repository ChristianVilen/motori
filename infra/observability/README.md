...
cert), with parquet offloaded to the private `motori-observability` R2 bucket (EU
jurisdiction). The `motori` app ships logs to it over a private `observability` Docker
network. **Dev** runs OO as a plain compose service in the root `docker-compose.yml`
(`docker compose up -d`).
