...
Cloudflare R2, EU jurisdiction, through the S3 API (`@aws-sdk/client-s3`, `region: "auto"`, `forcePathStyle: true`). Four buckets, one S3 API token per consumer, each scoped to only the buckets it needs (`motori-app`, `talli-app`, `dokku-backups`, `openobserve`; details in `DEPLOY.md` §13):
...
- **`motori-backups`** — **private**, the encrypted nightly Postgres dumps (`DEPLOY.md` §8). 30-day expiry and a 14-day bucket lock.
- **`motori-observability`** — **private**, OpenObserve's parquet (`DEPLOY.md` §11).

Env vars: `STORAGE_ENDPOINT`, `STORAGE_BUCKET`, `STORAGE_ACCESS_KEY`, `STORAGE_SECRET_KEY`, `STORAGE_PUBLIC_URL`, plus `STORAGE_DOCS_BUCKET` in talli. `STORAGE_ENDPOINT` doubles as the switch: unset (dev, CI) means local disk. The endpoint contains the Cloudflare account id. The repository is public, so every file in git writes `<ACCOUNT_ID>` and the real value lives only in `secrets/`. Bucket configuration (lifecycle, lock, custom domain) is done with wrangler from a laptop, never from the JS SDK and never from the VPS; see `infra/r2/`.
