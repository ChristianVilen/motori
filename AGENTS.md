...
**Stack:** TanStack Start (SSR + file-based routing) + React 19 + Kysely (Postgres) + BetterAuth + Tailwind v4 + Cloudflare R2 (S3-compatible, EU jurisdiction, `region: "auto"`, via `@aws-sdk/client-s3`) + sharp (server-side image optimisation).
...
The session cookie is scoped to `.motori.fi` in prod (`crossSubDomainCookies` in `createAuth`, disabled on localhost where subdomains don't apply) so the `talli.motori.fi` app shares the login (SSO). `trustedOrigins` includes `talli.motori.fi` / `localhost:3001` alongside the primary origin. talli mounts no auth routes of its own — it links to motori for sign-in/sign-up and relies on the shared cookie. Its one client auth call, sign-out, fetches motori cross-origin, so it needs three things lined up: motori's CORS allowlist (`apps/motori/src/lib/cors.ts`), motori's `trustedOrigins`, and talli's own CSP `connect-src` (the `connectSrc` option passed to `createSecurityHeadersMiddleware` in `apps/talli/src/start.ts`). Miss the last one and the browser blocks the fetch before it leaves the page.
...
Cloudflare R2, EU jurisdiction, through the S3 API (`@aws-sdk/client-s3`, `region: "auto"`, `forcePathStyle: true`). Four buckets, one S3 API token per consumer scoped to only the buckets it needs (token list, retention rules, alerts: `DEPLOY.md` §13):

- **`motori-images`** — the apps' `STORAGE_BUCKET`, public only through the custom domain `https://images.motori.fi` (= `STORAGE_PUBLIC_URL`; the `r2.dev` URL stays disabled). Listing photos, and talli's photos under the `talli/` prefix. Never store non-public data here.
- **`motori-docs`** — **private**, talli's per-vehicle documents (rekisteriote, insurance docs, etc.), served only via talli's authenticated `/api/documents/$id` proxy. `STORAGE_DOCS_BUCKET` is set in talli only; motori's config does not carry it. Never enable public read on it.
- **`motori-backups`** — **private**, the encrypted nightly Postgres dumps (`DEPLOY.md` §8).
- **`motori-observability`** — **private**, OpenObserve's parquet (`DEPLOY.md` §11).

Env vars: `STORAGE_ENDPOINT`, `STORAGE_BUCKET`, `STORAGE_ACCESS_KEY`, `STORAGE_SECRET_KEY`, `STORAGE_PUBLIC_URL`, plus `STORAGE_DOCS_BUCKET` in talli. `STORAGE_ENDPOINT` doubles as the switch: unset (dev, CI) means local disk. The endpoint contains the Cloudflare account id; the repository is public, so never commit it and write `<ACCOUNT_ID>` in examples (`DEPLOY.md` §13). Bucket configuration (lifecycle, lock, custom domain) is done with wrangler from a laptop, never from the JS SDK and never from the VPS; see `infra/r2/`.
...
**CI** runs on every PR and push to `main` (`.github/workflows/ci.yml`). Five parallel jobs: `build` (both apps), `lint`, `format`, `typecheck`, `test` (unit). Plus an `e2e` job sharded 2-way that spins up a Postgres 17 service container, runs migrations, builds, then runs Playwright against Chromium and WebKit, and an `e2e-talli` job that does the same for talli. The e2e jobs run inside the official Playwright container image (browsers and system deps preinstalled, so nothing downloads at job time) — when bumping `@playwright/test`, bump the `mcr.microsoft.com/playwright` image tag in `ci.yml` to the same version. E2e failures upload a `playwright-report` artifact (7-day retention). CI uses `.env.ci` (not `.env.example`) — keep that file in sync when adding required env vars. On `main`, a `deploy` job runs `scripts/deploy.sh` for each app (`just deploy` uses the same script). The script fails on a stale deploy lock and checks that `/api/health` returns the pushed commit as `version`. The job opens a "Deploy failed" issue if any step fails (`DEPLOY.md` §5b).
...
- Colour tokens in `packages/ui/src/theme.css` (`ring`, `destructive`, `success`, `warning`) are tuned to pass WCAG AA on the tinted surfaces they are used on (`bg-*/10`–`/20`), not just on white. Compute the contrast ratio before lightening any of them.
