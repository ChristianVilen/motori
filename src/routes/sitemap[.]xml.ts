...
const STATIC_PATHS = [
	...
	{ path: "/tori", priority: "0.9", changefreq: "daily" },
			...
			GET: async () => {
				...
				const urls = [
					...
					...toriItems.map((item) => {
						...
						return `<url><loc>${SITE_URL}/tori/${item.short_id}/${slug}</loc><lastmod>${new Date(item.updated_at).toISOString().split("T")[0]}</lastmod><changefreq>weekly</changefreq><priority>0.6</priority></url>`;
					}),
				];
