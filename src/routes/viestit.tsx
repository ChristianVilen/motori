...
function ViestitLayout() {
	...
	const inThread = router.state.location.pathname !== "/viestit";

	return (
		// Clip overflow so only one panel is visible at a time on mobile.
		// On md+ both panels show side by side as a normal flex row.
		<div className="overflow-hidden h-[calc(100dvh-120px)] md:h-[calc(100dvh-56px)] max-w-5xl mx-auto md:border-x border-border">
			{/*
			 * On mobile: 200% wide flex row — each panel is 50% = 100vw.
			 * Slide left by half (translate-x-[-50%]) to reveal the thread.
			 * On md+: normal width, no slide needed.
			 */}
			<div
				className={`flex h-full w-[200%] md:w-full transition-transform duration-300 ease-in-out ${
					inThread ? "-translate-x-1/2 md:translate-x-0" : "translate-x-0"
				}`}
			>
				<aside className="w-1/2 md:w-72 md:shrink-0 h-full border-r border-border bg-card flex flex-col">
				...
				<main className="w-1/2 md:flex-1 md:min-w-0 h-full bg-background">
					<Outlet />
				</main>
			</div>
		</div>
